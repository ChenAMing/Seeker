...
<template>
    ...
    <div class="flex flex-col gap-1 overflow-y-auto p-1">
        <button
            ...
            :key="item.name"
            @click="go(item.id)"
            class="flex min-h-[2rem] items-center gap-6 rounded px-3 transition-colors duration-300 hover:bg-mask active:bg-mask-more"
            :class="{ 'pointer-events-none bg-sec hover:bg-sec': route.query.id === item.id }">
            <span
                class="pointer-events-none !h-4 min-w-[1rem] transition-colors duration-150"
                :class="[
